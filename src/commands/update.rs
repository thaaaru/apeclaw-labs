...
const GITHUB_RELEASES_LATEST_URL: &str =
    "https://api.github.com/repos/apeclaw-labs/apeclaw/releases/latest";
const GITHUB_RELEASES_TAG_URL: &str =
    "https://api.github.com/repos/apeclaw-labs/apeclaw/releases/tags";
...
pub async fn check(target_version: Option<&str>) -> Result<UpdateInfo> {
    ...
    let client = reqwest::Client::builder()
        .user_agent(format!("apeclaw/{current}"))
...
pub async fn run(target_version: Option<&str>) -> Result<()> {
    ...
    let download_path = temp_dir.path().join("apeclaw_new");
...
async fn download_binary(url: &str, dest: &Path) -> Result<()> {
    let client = reqwest::Client::builder()
        .user_agent(format!("apeclaw/{}", env!("CARGO_PKG_VERSION")))
    ...
    // Release assets are .tar.gz archives containing a single `apeclaw` binary.
...
fn extract_tar_gz(archive_bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    bail!("archive does not contain a 'apeclaw' binary")
}
...
async fn validate_binary(path: &Path) -> Result<()> {
    ...
    if !stdout.contains("apeclaw") {
        bail!("downloaded binary does not appear to be apeclaw");
    }
...
mod tests {
    ...
    fn find_asset_url_picks_correct_gnu_over_android() {
        let release = make_release(&[
            "apeclaw-aarch64-linux-android.tar.gz",
            "apeclaw-aarch64-unknown-linux-gnu.tar.gz",
            "apeclaw-x86_64-unknown-linux-gnu.tar.gz",
            "apeclaw-x86_64-apple-darwin.tar.gz",
            "apeclaw-aarch64-apple-darwin.tar.gz",
        ]);
    ...
    fn extract_tar_gz_errors_on_missing_binary() {
        ...
        // Build a tar.gz with a file that is NOT named "apeclaw".
        let mut tar_buf = Vec::new();
        {
            let mut builder = tar::Builder::new(&mut tar_buf);
            let mut header = tar::Header::new_gnu();
