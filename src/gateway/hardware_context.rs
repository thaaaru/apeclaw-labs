...
//! which re-reads `~/.apeclaw/hardware/` from disk.  Writing to those files
...
/// Return `~/.apeclaw/hardware/` or an error string.
fn hardware_dir() -> Result<PathBuf, String> {
    directories::BaseDirs::new()
        .map(|b| b.home_dir().join(".apeclaw").join("hardware"))
...
/// `POST /api/hardware/pin` — register a single GPIO pin assignment.
///
/// Appends one line to `~/.apeclaw/hardware/devices/<device>.md`:
/// ```text
