...
Examples:
  apeclaw gateway start              # use config defaults
  apeclaw gateway start -p 8080      # listen on port 8080
  apeclaw gateway start --host 0.0.0.0   # requires [gateway].allow_public_bind=true or a tunnel
  apeclaw gateway start -p 0         # random available port")]
    Start {
...
Examples:
  apeclaw gateway restart            # restart with config defaults
  apeclaw gateway restart -p 8080    # restart on port 8080")]
    Restart {
        /// Port to listen on (use 0 for random available port); defaults to config gateway.port
        #[arg(short, long)]
        port: Option<u16>,
...
Examples:
  apeclaw gateway get-paircode       # show current pairing code
  apeclaw gateway get-paircode --new # generate a new pairing code")]
    GetPaircode {
...
Examples:
  ...
  apeclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  apeclaw channel bind-telegram 123456789")]
    BindTelegram {
...
Examples:
  apeclaw channel send 'Someone is near your device.' --channel-id telegram --recipient 123456789
  apeclaw channel send 'Build succeeded!' --channel-id discord --recipient 987654321")]
    Send {
...
pub enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `ApeClaw` workspace
    Openclaw {
...
Examples:
  apeclaw cron add-at 2025-01-15T14:00:00Z 'Send reminder'
  apeclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  apeclaw cron add-every 60000 'Ping heartbeat'     # every minute
  apeclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  apeclaw cron once 30m 'Run backup in 30 minutes'
  apeclaw cron once 2h 'Follow up on deployment'
  apeclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Treat the argument as an agent prompt instead of a shell command
...
Examples:
  apeclaw cron update <task-id> --expression '0 8 * * *'
  apeclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  apeclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  apeclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  apeclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
...
Examples:
  apeclaw peripheral flash
  apeclaw peripheral flash --port /dev/cu.usbmodem12345
  apeclaw peripheral flash -p COM3")]
    Flash {
