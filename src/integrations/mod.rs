...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `apeclaw onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: apeclaw onboard --channels-only");
        ...
        "Discord" => {
            ...
            println!("    4. Run: apeclaw onboard --channels-only");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.apeclaw/workspace/cron/");
