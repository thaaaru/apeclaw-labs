...
const OPEN_SKILLS_SYNC_MARKER: &str = ".apeclaw-open-skills-sync";
...
/// Skills live in `~/.apeclaw/workspace/skills/<name>/SKILL.md`
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.apeclaw/workspace/skills/my-skill");
                println!(
                    "              echo '# My Skill' > ~/.apeclaw/workspace/skills/my-skill/SKILL.md"
                );
                println!();
                println!("  Or install: apeclaw skills install <source>");
            } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.apeclaw");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.apeclaw/skills"));
    }
